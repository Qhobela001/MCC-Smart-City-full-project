...
def assign_incident(
    ...
    if not (
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Permission required: incidents.assign",
        )

    if incident.status in TERMINAL_STATUSES:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="A closed incident cannot be assigned.",
        )

    department_id = (
        payload.department_id
        if payload.department_id is not None
        else incident.department_id
    )
    ...
    if department_id is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                "Select a department before assigning "
                "the incident."
            ),
        )

    validate_department(db, department_id)
    assignee = validate_assignee(
        db,
        payload.assigned_user_id,
        department_id,
    )
    ...
    repository.add_activity(
        ...
        IncidentActivity(
            ...
            notes=(
                payload.notes
                or f"Assigned to {assignee.full_name}."
            ),
        ),
    )
