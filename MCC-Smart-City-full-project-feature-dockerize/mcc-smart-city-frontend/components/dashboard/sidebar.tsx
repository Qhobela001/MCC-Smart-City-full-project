...
export function Sidebar() {
    ...
    const groups = useMemo(() => {
        ...
        return [...map.entries()].sort(
            ([sectionA], [sectionB]) => {
                const priorityA =
                    sectionPriority[sectionA] ?? 999

                const priorityB =
                    sectionPriority[sectionB] ?? 999

                if (priorityA !== priorityB) {
                    return priorityA - priorityB
                }

                return sectionA.localeCompare(
                    sectionB,
                )
            },
        )
    ...
    /*
     * Whenever navigation is loaded, new sections
     * start expanded unless the user has already
     * manually changed that section.
     */
    useEffect(() => {
        if (groups.length === 0) {
            return
     ...
     * automatically expand the section containing
     * the current route.
     */
    useEffect(() => {
