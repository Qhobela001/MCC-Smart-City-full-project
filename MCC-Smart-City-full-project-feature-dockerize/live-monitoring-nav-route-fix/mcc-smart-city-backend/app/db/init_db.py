...
def init_db() -> None:
    ...
    with SessionLocal() as db:
        ...
        # Migrate the original Live Monitoring route to the real Next.js route.
        # This preserves the existing navigation item ID on already-seeded databases
        # and avoids creating duplicate Live Monitoring entries.
        legacy_live_monitoring = db.scalar(
            select(NavigationItem).where(
                NavigationItem.href == "/monitoring/live"
            )
        )
        current_live_monitoring = db.scalar(
            select(NavigationItem).where(
                NavigationItem.href == "/live-feeds"
            )
        )

        if legacy_live_monitoring is not None:
            if current_live_monitoring is None:
                legacy_live_monitoring.href = "/live-feeds"
            elif legacy_live_monitoring.id != current_live_monitoring.id:
                legacy_live_monitoring.is_active = False
